...
fun AddFxScreen(
    onNavigateBack: () -> Unit,
    viewModel: AddFxViewModel = hiltViewModel(),
) {
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()

    Scaffold(
        modifier = Modifier.testTag("addFxScreen"),
        topBar = {
            TopAppBar(
        ...
        Box(
            ...
            if (uiState.availableTracks.isEmpty()) {
                ...
                val addedNames = uiState.addedFx.map { it.name }.toSet()
                LazyColumn(
                    ...
                    items(uiState.availableTracks, key = { it.id }) { track ->
                        ...
                        Row(
                            ...
                            if (alreadyAdded) {
                                ...
                                IconButton(
                                    onClick = { viewModel.addFx(track.displayName) },
                                    modifier = Modifier.testTag("addFxButton_${track.displayName}"),
                                ) {
                                    Icon(
