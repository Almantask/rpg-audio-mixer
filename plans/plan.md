...
- `ExoPlayer` implementation for simple one-shot and loopable playback.
...
### Goal
Upgrade the audio engine to support multiple simultaneous looping tracks with per-category MIX volume and master volume.

### Build

**1. `SceneAudioEngine`**
- Holds a map of `categoryId → CategoryPlayer`
- Handles master volume and mix volume calculations

**2. `CategoryPlayer`**
- Manages playback for a specific soundscape category
- Supports intensity switching (I, II, III)
